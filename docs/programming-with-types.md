...
- Widening：
  - 8 位无符号 >>> 16 位无符号（后者可以表示所有前者）
- Narrowing：
  - 16 位无符号 >>> 8 位无符号 仅用于小类型
    - 需要显式，具有危险性！
- 隐藏与恢复类型信息
